...
*/
	...
	{
		string pathUpd = "bot=" + m_Unit.ToString();
		pathUpd = pathUpd + " dt=" + pDt.ToString();
		pathUpd = pathUpd + " sim=" + pSimulationPrecision.ToString();
		pathUpd = pathUpd + " recalc=" + BQExpansionDiag.B(m_Recalculate);
		pathUpd = pathUpd + " suppress=" + BQExpansionDiag.B(m_SuppressRecalculate);
		pathUpd = pathUpd + " target=" + m_TargetPosition.ToString();
		pathUpd = pathUpd + " end=" + GetEnd().ToString();
		pathUpd = pathUpd + " count=" + m_Count.ToString();
		pathUpd = pathUpd + " idx=" + m_PointIdx.ToString();
		pathUpd = pathUpd + " unreach=" + BQExpansionDiag.B(m_IsUnreachable);
		pathUpd = pathUpd + " targetUnreach=" + BQExpansionDiag.B(m_IsTargetUnreachable);
		pathUpd = pathUpd + " blocked=" + BQExpansionDiag.B(m_IsBlocked);
		pathUpd = pathUpd + " physBlocked=" + BQExpansionDiag.B(m_IsBlockedPhysically);
		BQExpansionDiag.Log("PATH_UPDATE", pathUpd);
	#ifdef EXTRACE_DIAG
		EXTrace trace;
		...
		if (recalculate)
		{
			string pathRecalc = "START bot=" + m_Unit.ToString();
			pathRecalc = pathRecalc + " unitPos=" + m_Unit.GetPosition().ToString();
			pathRecalc = pathRecalc + " target=" + m_TargetPosition.ToString();
			pathRecalc = pathRecalc + " allowJump=" + BQExpansionDiag.B(m_AllowJumpClimb);
			pathRecalc = pathRecalc + " allowDoors=" + BQExpansionDiag.B(m_AllowClosedDoors);
			BQExpansionDiag.Log("PATH_RECALC", pathRecalc);
			...
			*/
			...
			string afterSample = "AFTER_SAMPLE bot=" + m_Unit.ToString();
			afterSample = afterSample + " targetRef=" + m_TargetReference.GetPosition().ToString();
			afterSample = afterSample + " targetUnreach=" + BQExpansionDiag.B(m_IsTargetUnreachable);
			BQExpansionDiag.Log("PATH_RECALC", afterSample);
			m_Recalculate = false;
	...
	int GetNext(out vector position)
	{
		if (m_IsUnreachable)
		{
			string pathUnreach = "UNREACHABLE bot=" + m_Unit.ToString();
			pathUnreach = pathUnreach + " target=" + m_TargetPosition.ToString();
			pathUnreach = pathUnreach + " end=" + GetEnd().ToString();
			pathUnreach = pathUnreach + " count=" + m_Count.ToString();
			pathUnreach = pathUnreach + " idx=" + m_PointIdx.ToString();
			BQExpansionDiag.Log("PATH_NEXT", pathUnreach);
		...
		string pathNext = "bot=" + m_Unit.ToString();
		pathNext = pathNext + " next=" + position.ToString();
		pathNext = pathNext + " remaining=" + (m_Count - m_PointIdx).ToString();
		pathNext = pathNext + " count=" + m_Count.ToString();
		pathNext = pathNext + " idx=" + m_PointIdx.ToString();
		pathNext = pathNext + " target=" + m_TargetPosition.ToString();
		pathNext = pathNext + " end=" + GetEnd().ToString();
		BQExpansionDiag.Log("PATH_NEXT", pathNext);
	...
	void SetTarget(vector pPosition, float maxDistance = 1.0, bool allowJumpClimb = true)
	{
#ifdef EAI_TRACE
#endif
		string pathSet = "bot=" + m_Unit.ToString();
		pathSet = pathSet + " target=" + pPosition.ToString();
		pathSet = pathSet + " oldTarget=" + m_TargetPosition.ToString();
		pathSet = pathSet + " maxDist=" + maxDistance.ToString();
		pathSet = pathSet + " allowJump=" + BQExpansionDiag.B(allowJumpClimb);
		pathSet = pathSet + " override=" + BQExpansionDiag.B(m_OverridingPosition);
		pathSet = pathSet + " unreachable=" + BQExpansionDiag.B(m_IsUnreachable);
		pathSet = pathSet + " targetUnreach=" + BQExpansionDiag.B(m_IsTargetUnreachable);
		BQExpansionDiag.Log("PATH_SET", pathSet);
		...
		{
			BQExpansionDiag.Log("PATH_SET", "SKIP reason=overriding_position bot=" + m_Unit.ToString());
			return;
		}

		if (pPosition == m_TargetPosition)
		{
			BQExpansionDiag.Log("PATH_SET", "SAME bot=" + m_Unit.ToString() + " target=" + pPosition.ToString());
			m_SuppressRecalculate = false;
			return;
		}
