...
{
	protected int m_BQNextEAIWeaponMgrTraceTime;


	protected void BQ_EAIWeaponMgrTrace(string tag, string details = "", bool force = false)
	{
		if (!GetGame() || !GetGame().IsServer())
			return;

		int now = GetGame().GetTime();
		if (!force && now < m_BQNextEAIWeaponMgrTraceTime)
			return;

		m_BQNextEAIWeaponMgrTraceTime = now + 250;
		string msg = "[EAI_WEAPON] eAIWeaponManager " + tag;
		msg = msg + " t=" + now.ToString();
		if (m_player)
		{
			msg = msg + " player=" + m_player.GetType();
			msg = msg + " pos=" + m_player.GetPosition().ToString();
		}
		if (details != "")
			msg = msg + " " + details;
		Print(msg);
	...
	{
		BQ_EAIWeaponMgrTrace("StartAction", "action=" + action.ToString(), true);
	...
	{
		BQ_EAIWeaponMgrTrace("StartPendingAction", "action=" + m_PendingWeaponAction.ToString(), true);		
	...
	{
		string eventDump = "null";
		if (e)
			eventDump = e.DumpToString();
		BQ_EAIWeaponMgrTrace("PostWeaponEvent", "event=" + eventDump, true);
