class eAIState_TraversingWaypoints: eAIState
{
	...
	{
		Print("[EAI] [state_enter] bot=" + unit.ToString() + " state=eAIState_TraversingWaypoints event=" + Event);
		m_GotUp = false;
		unit.eAI_LeaveCover();
		...
		if (behaviour == eAIWaypointBehavior.ROAMING && m_Waypoint == vector.Zero)
		{
			...
			Print(string.Format("[EAI] [wp_roaming_pick] bot=%1 newRoamWp=%2 leaveThreshold=%3", unit.ToString(), m_Waypoint.ToString(), m_LeaveThreshold.ToString()));
		}
		Print(string.Format("[EAI] [wp_enter] bot=%1 behaviour=%2 currentWp=%3 wpIndex=%4", unit.ToString(), behaviour, m_Waypoint.ToString(), m_Index.ToString()));
	...
	{
		Print("[EAI] [state_exit] bot=" + unit.ToString() + " state=eAIState_TraversingWaypoints aborted=" + Aborted + " event=" + Event);
	...
	override int OnUpdate(float DeltaTime, int SimulationPrecision)
	{
		...
		if (waypointReached || (pathFinding.m_IsUnreachable && pathFinding.m_IsTargetUnreachable))
		{
			...
			else
			{
				...
				string backtrackStr = "false";
				if (backtracking)
					backtrackStr = "true";
				Print(string.Format("[EAI] [wp_advance] bot=%1 behaviour=%2 oldIndex=%3 newIndex=%4 backtracking=%5 pathCount=%6", unit.ToString(), behaviour, oldIndex.ToString(), m_Index.ToString(), backtrackStr, path.Count().ToString()));
		...
		if (pathFinding.m_IsUnreachable)
			unreachableStr = "true";
		string wpPrint = "[EAI] [wp_update] bot=" + unit.ToString();
		wpPrint = wpPrint + " wpIndex=" + m_Index.ToString();
		wpPrint = wpPrint + " target=" + m_Waypoint.ToString();
		wpPrint = wpPrint + " botPos=" + unit.GetPosition().ToString();
		wpPrint = wpPrint + " distSq=" + distance.ToString();
		wpPrint = wpPrint + " isFinal=" + isFinalStr;
		wpPrint = wpPrint + " unreachable=" + unreachableStr;
		wpPrint = wpPrint + " threshold=" + m_Threshold.ToString();
		Print(wpPrint);
