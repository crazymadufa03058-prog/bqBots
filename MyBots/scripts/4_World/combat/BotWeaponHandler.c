class BotWeaponHandler
{
	...
	void ResetAndChamber(Weapon_Base weapon)
	{
		...
		// 1. Сбрасываем FSM из WeaponFire → Idle
		weapon.ProcessWeaponEvent(new WeaponEventReloadTimeout(m_Bot));
		...
		if (weapon.CanFire(muzzle))
		{
			// ✅ Используем существующую переменную Magazine из верхней части функции
			Magazine burstMag = weapon.GetMagazine(muzzle);
			if (burstMag && burstMag.GetAmmoCount() > 0)  // Проверяем, есть ли патроны в магазине!
			{
			...
			{
				// Магазин пуст — начинаем перезарядку
				Print("[MyBots][FIRE] Magazine empty in burst, starting reload");
				StartReload(weapon);
				m_BurstCount = 0;
			}
		}
